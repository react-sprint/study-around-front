...
function StudyPageReadPresentation() {
  ...
  return (
    <Fragment>
      ...
      <Layout>
        <TabSelectLayout>
          <TabSelect onClick={() => setTab(1)} tabState={tab} tab={1}>
            소개
          </TabSelect>
          <TabSelect onClick={() => setTab(2)} tabState={tab} tab={2}>
            인증하기
          </TabSelect>
          <TabSelect onClick={() => setTab(3)} tabState={tab} tab={3}>
            설정
          </TabSelect>
        </TabSelectLayout>
        {tab === 1 && <IntroduceTab />}
        {tab === 2 && <CertificateTab />}
        {tab === 3 && <DeleteTab />}
      </Layout>
    </Fragment>
  );
...
const TabSelect = styled.div`
  ...
  border-bottom: ${(props) =>
    props.tab === props.tabState ? '2px solid #000' : '2px solid transparent'};
`;
